...
def prediction(characters, colm_list):
    classification_result = []
    plate_string = ''
    
    for each_character in characters:
        each_character = cv2.bitwise_not(each_character)
        each_character = each_character.reshape(1, -1); # converts it to a 1D array
        result = model.predict(each_character) # char matching
    ...
    column_list_copy = colm_list[:]
    colm_list.sort()
    rightplate_string = ''
    
    for each in colm_list:
...
def detect_possible_plate(frame): # start point of the lpr sys
    ...
    for box in regions:
        ...
        if pointYU > 220 and pointYL < 320: # if box located 320 & 333
            ...
            if len(final_list) >= 3: # if a plate consist at least 3 char
                ...
                characters, colm_list = DetectChars.recognizeCharsInPlate(rotate_img, final_list)    
                result = prediction(characters, colm_list)
                print(result)
                result_frame = frame
...
if __name__ == '__main__':
    try:
        ...
        try:
            model = joblib.load('./models/svc/svc.pkl') # read in training classifications
        except: # if file could not be opened
            print("error, unable to open svc.pkl, exiting program\n") # show error message
            os.system("pause") 
